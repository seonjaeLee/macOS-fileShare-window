...
### 5. GitHub 저장소 공개 전환 — 코드/문서 준비 완료, 실제 전환은 사용자 몫 (2026-07-07)
- 계정 권한이 필요한 작업(저장소 visibility 변경, Release 생성)이라 에이전트가 대신 실행할 수
  없음. `gh` CLI도 미설치 상태라 로그인 흐름을 거치기보다 GitHub 웹사이트에서 직접 하는 쪽으로
  결정함 (설치/인증 없이 바로 가능).
- 이번 세션에서 커밋·푸시까지 완료한 상태(커밋 `99a63ff`): README.md, LICENSE, DMG 배포 스크립트,
  x64 빌드 지원, 창 크기 수정, 클라이언트명 스크럽.
- 남은 수동 작업(사용자가 GitHub 웹에서 직접):
  1. Settings → Danger Zone → Change visibility → Public
  2. Releases → Create a new release (태그 예: `v1.0.0`) → `dist/한글 파일명 정리.dmg`,
     `dist/한글 파일명 정리 (Intel용).dmg` 두 파일을 에셋으로 첨부 → Publish
- `dist/`는 `.gitignore`에 포함되어 있어 두 `.dmg`는 로컬에만 있고 저장소에는 안 올라감 —
  Release 에셋 업로드로만 배포됨 (의도된 동작, 저장소 용량을 지킴).

### GitHub Release 첨부파일의 한글 파일명 깨짐 (2026-07-08)

사용자가 실제로 Release에 `한글 파일명 정리.dmg` / `한글 파일명 정리 (Intel용).dmg`를 두 번
드래그해서 올렸는데, 두 번 다 GitHub이 자동으로 `default.dmg` / `Intel.dmg`로 바꿔버림.
(한글이 전부 빠지고, 영문자가 남아있던 파일은 그 영문자만 남고, 영문자가 하나도 없던 파일은
`default`로 대체됨 — 비ASCII 문자를 통째로 제거하는 패턴과 정확히 일치)

**이 앱이 원래 해결하려는 문제(맥→윈도우 한글 파일명 깨짐)와 증상은 닮았지만 원인은 완전히
다름**: 이번엔 macOS나 zip이 아니라 GitHub Release 첨부파일 업로드 자체가 비ASCII 파일명을
지원하지 않는 것으로 재현됨(같은 파일을 두 번 다른 방식으로 시도해도 동일하게 재현). 클라이언트
(우리 앱) 쪽에서 손쓸 수 있는 부분이 아니라서, 배포용 `.dmg` 파일명만 영문으로 바꾸는 쪽으로
대응함.

**수정** ([build-dmg.sh](build-dmg.sh)):
- 기존엔 `.dmg` 파일명이 `.app` 이름(한글, "한글 파일명 정리")을 그대로 따라갔음.
- `ASCII_BASE="$(basename "$(pwd)")"` (=프로젝트 폴더명이자 GitHub 저장소 이름인
  `macOS-fileShare-window`)를 사용해 최종 파일명만 `macOS-fileShare-window.dmg` /
  `macOS-fileShare-window-intel.dmg`로 영문화. 처음엔 `package.json`의 `name`
  (`hangul-filename-fixer`)을 쓰려 했으나, 사용자가 "저장소 이름과 통일하자"고 판단해 저장소
  이름 기준으로 변경.
- DMG를 더블클릭해서 열었을 때 Finder에 보이는 **볼륨 이름(volname)**은 그대로 한글
  ("한글 파일명 정리")로 유지 — 이건 로컬 파일시스템 안에서 마운트되는 것이라 GitHub 업로드와
  무관하고, 한글이 정상 표시되므로 사용성을 위해 유지함. **파일명만 영문, 열었을 때 보이는
  이름은 한글**인 셈.
- [README.md](README.md)의 설치 안내도 새 파일명(`macOS-fileShare-window.dmg`,
  `macOS-fileShare-window-intel.dmg`)에 맞게 갱신.
- **검수**: `./build-dmg.sh` 재실행 후 `dist/`에 의도한 영문 파일명으로 정확히 생성되는 것을
  확인. GitHub에 실제로 재업로드해서 파일명이 안 깨지는지는 사용자가 직접 확인 예정.
